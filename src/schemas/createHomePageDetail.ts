...
export const CreateHomePageDetail: ZodType<CreatHomePageDetailType> = z.object({
  ...
  banner_image_url: z.array(z.unknown()).min(1, { message: "At least one banner image URL is required" }),
  content_01_title: z.string().min(1, { message: "Content title is required" }),
  content_01_detail: z
    .string()
    .min(1, { message: "Content detail is required" }),
  content_02_image_url: z.array(z.unknown()).min(1, { message: "At least one content 2 image URL is required" }),
  content_02_detail: z
    .string()
    .min(1, { message: "Content 2 detail is required" }),
  contact_image_url: z.array(z.unknown()).min(1, { message: "At least one contact image URL is required" }),
